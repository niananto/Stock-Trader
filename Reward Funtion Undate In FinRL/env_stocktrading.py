...
class StockTradingEnv(gym.Env):
    ...
    def __init__(
        ...
        state_space: int,
        action_space: int,
        exponential_cash_average: float,
        ...
        self.exponential_cash_average = exponential_cash_average
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            self.cash = (1-self.exponential_cash_average) * self.cash + self.exponential_cash_average * self.state[0]
        ...
        #         additional_reward = dot(holdings,positive_profit_sell_diff_avg_buy)
            ...
            #     * np.array(self.state[(self.stock_dim + 1) : (self.stock_dim * 2 + 1)])
            # )
            # low_profit_penalty = -1 * sum(
            #     np.array(self.state[1 : (self.stock_dim + 1)])
            #     * np.array(self.state[(self.stock_dim + 1) : (self.stock_dim * 2 + 1)])
            # )
            # stop_loss_penalty = -1 * sum(
            #     np.array(self.state[1 : (self.stock_dim + 1)])
            #     * np.array(self.state[(self.stock_dim + 1) : (self.stock_dim * 2 + 1)])
            # )
            # cash_penalty = cash_penalty = max(0, (self.cash + end_total_asset - self.state[0]) * self.cash_penalty_percentage - self.cash)
            # total_penalty = cash_penalty + stop_loss_penalty + low_profit_penalty

            # self.reward = (self.cash+ end_total_asset- self.state[0] + additional_reward - total_penalty - self.initial_amount) / self.initial_amount / self.day
